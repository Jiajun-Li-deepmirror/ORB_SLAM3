                ...
                vector<string> &vstrImageLeft, vector<string> &vstrImageRight, vector<double> &vTimeStamps)
{
    ...
    while(!fTimes.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            // OpenLORIS-Scene's fisheye{1,2}.txt timestamps are already fractional seconds
            // (unlike TUM-VI's nanosecond integers), so no /1e9 conversion here.
            double t = stod(item);
...
void LoadIMU(const string &strImuPath, vector<double> &vTimeStamps, vector<cv::Point3f> &vAcc, vector<cv::Point3f> &vGyro)
{
    ...
    while(!fImu.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            // sync_imu.py's imu_synced.txt column layout matches TUM-VI's imu.txt exactly
            // (timestamp,gx,gy,gz,ax,ay,az) except the timestamp is already fractional seconds.
